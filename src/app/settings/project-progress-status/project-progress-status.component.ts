...
@Component({
  selector: 'project-progress-status',
  imports: [
    CommonModule,
    FormsModule,
    ReactiveFormsModule,
  ...
  styleUrl: './project-progress-status.component.scss'
})
export class ProjectProgressStatusComponent implements OnInit {
  ...
  loadData(): void {
    this.loading = true;
    this.managementService.findEtats()
  ...
  saveStatus() {
    ...
    if (!this.selectedItem?.id) {
      ...
      this.managementService.saveEtat(etat)
        .pipe(takeUntilDestroyed(this.destroyRef))
        .subscribe({
          next: (data: any) => {
...
// Update status
  updateStatus() {
    ...
    this.managementService.updateEtat(etat)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (etat: any) => {
  ...
  deleteEtat(): void {
    if (confirm('Veuillez confirmer cette suppression !')) {
      this.managementService.deleteEtat(this.selectedItem?.id).pipe(
