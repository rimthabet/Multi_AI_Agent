...
@Component({
  selector: 'app-bar-chart-09',
  imports: [HighchartsChartModule],
  templateUrl: './bar-chart-09.component.html',
  styleUrl: './bar-chart-09.component.scss'
})
export class BarChart09Component implements AfterViewInit, OnChanges {
  /// inputs  
  ...
  fetchDataHistory() {
    ...
    const bpData = this.bp();
    const itemData = this.item();

    if (!prospectionData || !bpData || !itemData) {
      return;
    }
    this.loading = true;
    this.years = [
  ...
  updateChart(): void {
    ...
    const seriesData = this.years.map((year, index) => {
      ...
      // Create temp object to substitute the null returned objects
      if (fetchedData == null) {
        fetchedData = {
          year: year,
          value: 0,
